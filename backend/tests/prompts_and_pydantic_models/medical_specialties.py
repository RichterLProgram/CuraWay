...
try:
    ...
    # Fallback when fdr.config not available (e.g. CancerCompass standalone)
    _SPECIALTIES = [
        ...
        "ophthalmology", "pathology", "medicalOncology", "radiology", "nephrology",
        "infectiousDiseases", "psychiatry", "urology", "otolaryngology", "plasticSurgery",
        "criticalCareMedicine", "physicalMedicineAndRehabilitation", "anesthesia",
        "hospiceAndPalliativeInternalMedicine",
    ]
    MEDICAL_HIERATCHY = {"children": [{"name": s} for s in _SPECIALTIES]}
    def flatten_specialties_to_level(hierarchy, level):
        return _SPECIALTIES
