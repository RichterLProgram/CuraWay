...
from features.clinical_trials.models import EligibilitySignal, TrialMatch, TrialRecord
from features.clinical_trials.routing import estimate_travel_time_minutes
...
def match_trials(
    ...
    for trial in trials:
        ...
        travel_time = _travel_time_to_closest_site(patient, trial)
...
def _travel_time_to_closest_site(
    patient: PatientProfile, trial: TrialRecord
) -> Optional[float]:
    if not patient.location or patient.location.latitude is None or patient.location.longitude is None:
        return None
